...
function vegosszeg(){
    ...
    if (osszesfo > 5) potagyszam = osszesfo - 4;
    ...
    if(osszesfo >= 5){
        hibauzenet.innerHTML = "A foglalás nem haladhatja meg az 5 főt";
        hibauzenet.style.animationName = "hiba";
    }
    else{
        hibauzenet.style.animationName = "blank";
...
function felnottF(){
    if(felnott.value <= 0 || felnott.value > 4){
        felnott.value = null;
    }
    if(Number(felnott.value) + Number(gyerek1.value) + Number(gyerek2.value) > 5)
        felnott.value -= 1;
}
function gyerek1F(){
    if(gyerek1.value < 0 || gyerek1.value > 4){
        gyerek1.value = null;
    }
    if(Number(felnott.value) + Number(gyerek1.value) + Number(gyerek2.value) > 5)
        gyerek1.value -= 1;
...
function gyerek2F(){
    if(gyerek2.value < 0 || gyerek2.value > 4){
        gyerek2.value = null;
    }
    if(Number(felnott.value) + Number(gyerek1.value) + Number(gyerek2.value) > 5)
        gyerek2.value -= 1;
